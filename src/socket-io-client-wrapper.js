...
class SocketIOClientWrapper {
  constructor({
    ...
    console.log(this.isInitiator, this.sessionId);

    this._initSocket();
    this._onCreatedCallback = null;
    this._onPeerJoinedCallback = null;
  }
  ...
  _startSocketCommunication() {
    ...
    if (this.isInitiator) {
      ...
      this.debug && console.log('Attempted to create room');
    } else {
      this.socket.emit('join', this.sessionId);
      this.debug && console.log('Attempted to join room');
    }
  ...
  _handleCreated(room) {
    ...
    if (this._onCreatedCallback) this._onCreatedCallback(room);
  }
  ...
  _handleJoinRoom(room) {
    ...
    if (this.peerClient.initPeerRequest) {
      ...
      if (this._onPeerJoinedCallback) this._onPeerJoinedCallback();
    }
  }
